...
- If there are `CustomerOrders` in the queue, it will verify the fill status of the **last** `CustomerOrder`
     ...
     if any item in the customer order is done by the task, then use the fillitem function
     */
...
void Task::validate(std::ostream& os)
{
    if(m_pNextTask == nullptr){
        ...
        os << this->getName() << " --> "  << this->m_pNextTask->getName() << std::endl;
    }
...
{
    m_orders.push_front(std::move(moveCustOrd));
    return *this;
}